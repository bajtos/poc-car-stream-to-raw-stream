...
// Runtime types generated with workerd@1.20250924.0 2025-09-24
 ...
 * API, but with some differences in the [supported
...
/**
 * The Cache API allows fine grained control of reading and writing from the
...
/**
 * The Workers runtime supports a subset of the Performance API, used to measure
 * timing and performance, as well as timing of subrequests and other
 * operations.
 *
 * [Cloudflare Docs
 ...
 * global scope as part of the service worker lifecycle. This ensures that any
 * functional events (like FetchEvent) are not dispatched until it upgrades
 ...
 * global scope. It contains information about the fetch, including the request
 * and how the receiver will treat the response. It provides the
 * event.respondWith() method, which allows us to provide a response to this
 * fetch.
 *
...
type Ai_Cf_Baai_Bge_Base_En_V1_5_Input =
	| {
			text: string | string[]
			/**
			 * The pooling method used in the embedding process. `cls` pooling will
			 * generate more accurate embeddings on larger inputs - however,
			 * embeddings created with cls pooling are not compatible with embeddings
			 * generated with mean pooling. The default pooling method is `mean` in
			 * order for this to not be a breaking change, but we highly suggest using
			 * the new `cls` pooling for better accuracy.
			 */
			pooling?: 'mean' | 'cls'
	  }
	| {
			...
			requests: {
				 ...
				 * The pooling method used in the embedding process. `cls` pooling will
				 * generate more accurate embeddings on larger inputs - however,
				 * embeddings created with cls pooling are not compatible with
				 * embeddings generated with mean pooling. The default pooling method is
				 * `mean` in order for this to not be a breaking change, but we highly
				 * suggest using the new `cls` pooling for better accuracy.
				 */
				pooling?: 'mean' | 'cls'
			}[]
	  }
...
type Ai_Cf_Openai_Whisper_Input =
	...
	| {
			/**
			 * An array of integers that represent the audio data constrained to 8-bit
...
type Ai_Cf_Meta_M2M100_1_2B_Input =
	| {
			...
			text: string
			/**
			 * The language code of the source text (e.g., 'en' for English). Defaults
...
interface Ai_Cf_Openai_Whisper_Large_V3_Turbo_Input {
	...
	vad_filter?: boolean
	/**
	 * A text prompt to help provide context to the model on the contents of the
	 * audio.
	 */
...
interface Ai_Cf_Openai_Whisper_Large_V3_Turbo_Output {
	transcription_info?: {
		...
		language?: string
		/**
		 * The confidence level or probability of the detected language being
		 * accurate, represented as a decimal between 0 and 1.
		 */
	...
	segments?: {
		...
		temperature?: number
		/**
		 * The average log probability of the predictions for the words in this
		 * segment, indicating overall confidence.
		 */
		avg_logprob?: number
		/**
		...
		/**
		 * The probability that the segment contains no speech, represented as a
	...
	/**
	 * The transcription in WebVTT format, which includes timing and text
	 * information for use in subtitles.
	 */
	vtt?: string
}
...
interface Prompt {
	...
	temperature?: number
	/**
	 * Adjusts the creativity of the AI's responses by controlling how many
...
interface AsyncBatch {
	requests?: {
		/**
		 * User-supplied reference. This field will be present in the response as
		 * well it can be used to reference the request and response. It's NOT
...
interface Ai_Cf_Meta_Llama_Guard_3_8B_Input {
	...
	messages: {
		/**
		 * The role of the message sender must alternate between 'user' and
		 * 'assistant'.
		 */
		role: 'user' | 'assistant'
		/** The content of the message as a string. */
...
interface Ai_Cf_Meta_Llama_Guard_3_8B_Output {
	response?:
		...
		| {
				...
				safe?: boolean
				/**
				 * A list of what hazard categories predicted for the conversation, if
				 * the conversation is deemed unsafe.
				 */
				categories?: string[]
		  }
	/** Usage statistics for the inference request */
...
interface Ai_Cf_Meta_Llama_4_Messages {
	...
	messages: {
		...
		role?: string
		/**
		 * The tool call id. If you don't know what to put here you can fall back to
		 * 000000001
		 */
		tool_call_id?: string
		content?:
...
interface Ai_Cf_Deepgram_Nova_3_Input {
	 ...
	 * param. When strict, the model will only return topics submitted using the
	 * custom_topic param. When extended, the model will return its own detected
	 * topics in addition to those submitted using the custom_topic param.
	 */
	custom_topic_mode?: 'extended' | 'strict'
	/**
	 * Custom topics you want the model to detect within your input audio or text
	 * if present Submit up to 100
	 */
	custom_topic?: string
	/**
	 * Sets how the model will interpret intents submitted to the custom_intent
	 * param. When strict, the model will only return intents submitted using the
	 * custom_intent param. When extended, the model will return its own detected
	 * intents in addition those submitted using the custom_intents param
	 */
	custom_intent_mode?: 'extended' | 'strict'
	/**
	 * Custom intents you want the model to detect within your input audio if
	 * present
	 */
	...
	encoding?:
		...
		| 'g729'
	/**
	 * Arbitrary key-value pairs that are attached to the API response for usage
	 ...
	 * Depending on the Model and API endpoint you choose only certain languages
	 * are available.
	 */
	language?: string
	...
	/**
	 * Opts out requests from the Deepgram Model Improvement Program. Refer to our
	 * Docs for pricing impacts before setting this to true.
	 * https://dpgr.am/deepgram-mip.
	 */
	mip_opt_out?: boolean
	/**
	 * Mode of operation for the model representing broad area of topic that will
	 * be talked about in the supplied audio
	 */
	 ...
	 * endpoint sends continuous updates, meaning transcription results may evolve
	 * over time. Note: Supported only for webosockets.
	 */
	interim_results?: boolean
	/**
	 * Indicates how long model will wait to detect whether a speaker has finished
	 * speaking or pauses for a significant period of time. When set to a value,
	 * the streaming endpoint immediately finalizes the transcription for the
	 * processed time range and returns the transcript with a speech_final
	 * parameter set to true. Can also be set to false to disable endpointing
	 */
	endpointing?: string
	/**
	 * Indicates that speech has started. You'll begin receiving Speech Started
	 * messages upon speech starting. Note: Supported only for webosockets.
	 */
	vad_events?: boolean
	/**
	 * Indicates how long model will wait to send an UtteranceEnd message after a
	 * word has been transcribed. Use with interim_results. Note: Supported only
	 * for webosockets.
	 */
...
type Ai_Cf_Pipecat_Ai_Smart_Turn_V2_Input =
	| {
			/**
			 * Readable stream with audio data and content-type specified for that
			 * data
			 */
			audio: {
				body: object
...
interface GPT_OSS_120B_Responses {
	/**
	 * Responses API Input messages. Refer to OpenAI Responses API docs to learn
	 * more about supported content types
	 */
...
interface Ai_Cf_Leonardo_Phoenix_1_0_Input {
	 ...
	 * higher values make the image more aligned with the prompt
	 */
	guidance?: number
...
interface Ai_Cf_Deepgram_Aura_1_Input {
	...
	/**
	 * Container specifies the file format wrapper for the output audio. The
	 * available options depend on the encoding type..
	 */
	container?: 'none' | 'wav' | 'ogg'
	/** The text content to be converted to speech */
	text: string
	/**
	 * Sample Rate specifies the sample rate for the output audio. Based on the
	 * encoding, different sample rates are supported. For some encodings, the
	 * sample rate is not configurable
	 */
	sample_rate?: number
	/**
	 * The bitrate of the audio in bits per second. Choose from predefined ranges
	 * or specific values based on the encoding type.
	 */
	bit_rate?: number
}
...
/**
 * In addition to the properties you can set in the RequestInit dict that you
 * pass as an argument to the Request constructor, you can set certain
 * properties of a `cf` object to control how Cloudflare features are applied to
 * that new Request.
 *
...
interface RequestInitCfProperties extends Record<string, unknown> {
	...
	cacheTtl?: number
	/**
	 * Force response to be cached for a given number of seconds based on the
	 * Origin status code. (e.g. { '200-299': 86400, '404': 1, '500-599': 0 })
	 */
...
interface RequestInitCfPropertiesImageDraw extends BasicImageTransformations {
	...
	opacity?: number
	/**
	 * - If set to true, the overlay image will be tiled to cover the entire area.
	 *   This is useful for stock-photo-like watermarks.
	 * - If set to "x", the overlay image will be tiled horizontally only (form a
	 *   line).
	 * - If set to "y", the overlay image will be tiled vertically only (form a
	 *   line).
	 */
	repeat?: true | 'x' | 'y'
	/**
...
interface RequestInitCfPropertiesImage extends BasicImageTransformations {
	...
	dpr?: number
	/**
	 * Allows you to trim your image. Takes dpr into account and is performed
	 * before resizing or rotation.
	 *
	 * It can be used as:
	 *
	 ...
	 *   any.
	 * - Copyright: Only keep the copyright tag, and discard everything else. This
	 *   is the default behavior for JPEG files.
	 * - None: Discard all invisible EXIF metadata. Currently WebP and PNG output
	 *   formats always discard metadata.
	 */
	...
	/**
	 * Fetching image from authenticated origin. Setting this property will pass
	 * authentication headers (Authorization, Cookie, etc.) through to the
	 * origin.
	 */
	...
	saturation?: number
	/**
	 * Flips the images horizontally, vertically, or both. Flipping is applied
	 * before rotation, so if you apply flip=h,rotate=90 then the image will be
	 * flipped horizontally, then rotated by 90 degrees.
	 */
	flip?: 'h' | 'v' | 'hv'
	/**
...
interface IncomingRequestCfPropertiesBase extends Record<string, unknown> {
	...
	/**
	 * The original value of the `Accept-Encoding` header if Cloudflare modified
	 * it.
	 *
	...
	colo: string
	/**
	 * Represents the upstream's response to a [TCP `keepalive`
	 * message](https://tldp.org/HOWTO/TCP-Keepalive-HOWTO/overview.html) from
	 * cloudflare.
	 *
	...
	/**
	 * The TLS version of the connection to Cloudflare. In requests served over
	 * plaintext (without TLS), this property is the empty string `""`.
	 *
	 * @example TLSv1.3
	 */
	tlsVersion: string
	/**
	 * The cipher for the connection to Cloudflare. In requests served over
	 * plaintext (without TLS), this property is the empty string `""`.
	 *
...
interface IncomingRequestCfPropertiesBotManagementBase {
	...
	score: number
	/**
	 * A boolean value that is true if the request comes from a good bot, like
	 * Google or Bing. Most customers choose to allow this traffic. For more
	 * details, see [Traffic from known
	...
	/**
	 * A boolean value that's true if the request matches [file
	 * extensions](https://developers.cloudflare.com/bots/reference/static-resources/)
	 * for many types of static resources.
	 */
...
interface IncomingRequestCfPropertiesBotManagementEnterprise
	...
	botManagement: IncomingRequestCfPropertiesBotManagementBase & {
		 ...
		 * to help profile specific SSL/TLS clients across different destination
...
interface IncomingRequestCfPropertiesCloudflareAccessOrApiShield {
	 ...
	 *
	 * The property `certPresented` will be set to `"1"` when the object is
	 * populated (i.e. the above conditions were met).
	 */
	tlsClientAuth:
...
interface IncomingRequestCfPropertiesGeographicInformation {
	/**
	 * The [ISO 3166-1 Alpha 2](https://www.iso.org/iso-3166-country-codes.html)
	 * country code the request originated from.
	 *
	 * If your worker is [configured to accept TOR
	 * connections](https://support.cloudflare.com/hc/en-us/articles/203306930-Understanding-Cloudflare-Tor-support-and-Onion-Routing),
	 * this may also be `"T1"`, indicating a request that originated over TOR.
	 *
	 * If Cloudflare is unable to determine where the request originated this
	 * property is omitted.
	 *
...
interface IncomingRequestCfPropertiesTLSClientAuth {
	...
	certRevoked: '1' | '0'
	/**
	 * The certificate issuer's [distinguished
...
interface D1Meta {
	...
	/**
	 * True if-and-only-if the database instance that executed the query was the
	 * primary.
	 */
	served_by_primary?: boolean
	timings?: {
		/**
		 * The duration of the SQL query execution by the database instance. It
		 * doesn't include any network time.
		 */
		sql_duration_ms: number
	}
	/**
	 * Number of total attempts to execute the query, due to automatic retries.
	 * Note: All other fields in the response like `timings` only apply to the
	 * last attempt.
	 */
...
declare abstract class D1Database {
	...
	/**
	 * @deprecated Dump() will be removed soon, only applies to deprecated alpha
	 *   v1 databases.
	 */
...
declare abstract class D1DatabaseSession {
	...
	batch<T = unknown>(statements: D1PreparedStatement[]): Promise<D1Result<T>[]>
	/**
	 * @returns The latest session bookmark across all executed queries on the
	 *   session. If no query has been executed yet, `null` is returned.
	 */
...
/**
 * An email message that is sent to a consumer Worker and can be
 * rejected/forwarded.
 */
interface ForwardableEmailMessage extends EmailMessage {
	...
	/**
	 * An [Headers
	 * object](https://developer.mozilla.org/en-US/docs/Web/API/Headers).
	 */
	readonly headers: Headers
	/** Size of the email message content. */
	readonly rawSize: number
	/**
	 * Reject this email message by returning a permanent SMTP error back to the
	 * connecting client including the given reason.
	 *
	...
	/**
	 * Forward this email message to a verified destination address of the
	 * account.
	 *
...
interface Hyperdrive {
	/**
	 * Connect directly to Hyperdrive as if it's your database, returning a TCP
	 * socket.
	 *
...
interface ImageTransformer {
	...
	/**
	 * Draw an image on this transformer, returning a transform handle. You can
	 * then apply more transformations, draw, or retrieve the output.
	 *
...
/**
 * Media binding for transforming media streams. Provides the entry point for
 * media transformation operations.
 */
...
/**
 * Generator for producing media transformation results. Configures the output
 * format and parameters for the transformed media.
 */
...
/**
 * Result of a media transformation operation. Provides multiple ways to access
...
interface MediaTransformationResult {
	 ...
	 *
	 * @returns The transformed media as a Response, ready to store in cache or
	 *   return to users
	 */
	response(): Response
	/**
...
/**
 * Configuration options for transforming media input. Controls how the media
 * should be resized and fitted.
 */
...
/**
 * Configuration options for Media Transformations output. Controls the format,
 * timing, and type of the generated output.
 */
type MediaTransformationOutputOptions = {
	...
	audio?: boolean
	/**
	 * Starting timestamp for frame extraction or start time for clips. (e.g.
	 * '2s').
	 */
	time?: string
	/**
	 * Duration for video clips, audio extraction, and spritesheet generation
	 * (e.g. '5s').
	 */
	duration?: string
...
/**
 * Error object for media transformation operations. Extends the standard Error
 * interface with additional media-specific information.
 */
...
declare module 'cloudflare:pipelines' {
	...
	> {
		...
		/**
		 * Run recieves an array of PipelineRecord which can be transformed and
		 * returned to the pipeline
		 *
		 * @param records Incoming records from the pipeline to be transformed
		 * @param metadata Information about the specific pipeline calling the
		 *   transformation entrypoint
...
/**
 * Filter criteria for vector metadata used to limit the retrieved query result
 * set.
 */
...
/**
 * Supported distance metrics for an index. Distance metrics determine how other
 * "similar" vectors are determined.
 */
 ...
 * Default to "none".
 *
 * @property all Full metadata for the vector return set, including all fields
 *   (including those un-indexed) without truncation. This is a more expensive
 *   retrieval, as it requires additional fetching & reading of un-indexed
 *   data.
 * @property indexed Return all metadata fields configured for indexing in the
 *   vector return set. This level of retrieval is "free" in that no additional
 *   overhead is incurred returning this data. However, note that indexed
 *   metadata is subject to truncation (especially for larger strings).
 ...
 *
 * This type is exclusively for the Vectorize **beta** and will be deprecated
 * once Vectorize RC is released. See {@link VectorizeIndexInfo} for its
 * post-beta equivalent.
 */
...
interface VectorizeIndexInfo {
	...
	dimensions: number
	/**
	 * ISO 8601 datetime of the last processed mutation on in the index. All
	 * changes before this mutation will be reflected in the index state.
	 */
...
interface VectorizeVector {
	/**
	 * The ID for the vector. This can be user-defined, and must be unique. It
	 * should uniquely identify the object, and is best set based on the ID of
	...
	namespace?: string
	/**
	 * Metadata associated with the vector. Includes the values of other fields
	 * and potentially additional details.
	 */
	metadata?: Record<string, VectorizeVectorMetadata>
}
/**
 * Represents a matched vector for a query along with its score and (if
 * specified) the matching vector information.
 */
...
/**
 * Results of an operation that performed a mutation on a set of vectors. Here,
 * `ids` is a list of vectors that were successfully processed.
 *
 * This type is exclusively for the Vectorize **beta** and will be deprecated
 * once Vectorize RC is released. See {@link VectorizeAsyncMutation} for its
 * post-beta equivalent.
 */
...
/**
 * Result type indicating a mutation on the Vectorize Index. Actual mutations
 * are processed async where the `mutationId` is the unique identifier for the
 * operation.
 */
interface VectorizeAsyncMutation {
	/**
	 * The unique identifier for the async mutation operation containing the
	 * changeset.
	 */
	mutationId: string
}
 ...
 *
 * This type is exclusively for the Vectorize **beta** and will be deprecated
 * once Vectorize RC is released. See {@link Vectorize} for its new
 * implementation.
 */
declare abstract class VectorizeIndex {
	 ...
	 *
	 * @param vector Input vector that will be used to drive the similarity
	 *   search.
	...
	/**
	 * Insert a list of vectors into the index dataset. If a provided id exists,
	...
	/**
	 * Upsert a list of vectors into the index dataset. If a provided id exists,
...
declare abstract class Vectorize {
	 ...
	 *   containing the insert changeset.
	 */
	 ...
	 * @param vectors List of vectors that will be upserted.
	 * @returns A promise that resolves with a unique identifier of a mutation
	 *   containing the upsert changeset.
	 */
	 ...
	 * @param ids List of vector ids that should be deleted.
	 * @returns A promise that resolves with a unique identifier of a mutation
	 *   containing the delete changeset.
	 */
...
/**
 * The interface for "version_metadata" binding providing metadata about the
 * Worker Version using this binding.
 */
...
declare module 'cloudflare:workflows' {
	/**
	 * NonRetryableError allows for a user to throw a fatal error that makes a
	 * Workflow instance fail immediately without triggering a retry
	 */
...
declare abstract class Workflow<PARAMS = unknown> {
	...
	/**
	 * Create a new instance and return a handle to it. If a provided id exists,
	 * an error will be thrown.
	 *
	...
	/**
	 * Create a batch of instances and return handle for all of them. If a
	 * provided id exists, an error will be thrown. `createBatch` is limited at
	 * 100 instances at a time or when the RPC limit for the batch (1MiB) is
	 * reached.
	 *
	 * @param batch List of Options when creating an instance including name and
	 *   params
	 * @returns A promise that resolves with a list of handles for the created
	 *   instances.
	 */
	public createBatch(
...
interface WorkflowInstanceCreateOptions<PARAMS = unknown> {
	...
	params?: PARAMS
	/**
	 * The retention policy for Workflow instance. Defaults to the maximum
	 * retention period available for the owner's account.
	 */
	retention?: {
